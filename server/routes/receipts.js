...
router.post('/upload', requireAuth, requireOwner, upload.array('pdfs', 20), async (req, res) => {
  ...
  // Load QBO reference data + product memory once for all files
  const [accountsRes, classesRes, memoryRes] = await Promise.all([
  ...
  for (const file of req.files) {
    ...
    try {
      ...
      if (items?.length && accounts.length) {
        try {
          categorized = await categorizeLineItems(items, accounts, classes, memory);
        } catch (catErr) {
